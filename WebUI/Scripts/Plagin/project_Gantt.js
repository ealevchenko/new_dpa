(function ($) {
    ...
    var name_plagin = "prjGantt";
    // значение по умолчанию
    var defaults = {
        language: 'auto',                   // локализация
        open: true,                         // открытые ветки
    ...
    var methods = {
        ...
        init: function (params) {
            ...
            return this.each(function (i, el) {
                ...
                if (!data.id) {
                    var name_gantt = 'GanttChartDIV-' + getID($this);
                    $(this).data('id', name_gantt);
                    var option;
                    ...
                    option = $.extend(true,
                        ...
                        params,
                        {
                            id_prj_Gantt: name_gantt
                        }
                    );
                    $(this).data('option', option);

                    // Справочник лаколизации
                    var localization = $.extend(true, getLanguages($.gantt_Common, option.language));
                    $(this).data('localization', localization);
                    //$this.addClass('gantt');
                    //$this.attr('id', name_gantt);
                    //$this.attr('style', "position:relative");
                    $this.append('<div style="position:relative" class="gantt" id="' + name_gantt + '"></div>');
                    //$this.append('<script>var g' + i+' = null;</script>');
                    //g = new JSGantt.GanttChart('g', document.getElementById(name_gantt), 'day');
                    g = new JSGantt.GanttChart('g' + i, document.getElementById(name_gantt), 'day');
                    g.setShowRes(1); // Show/Hide Responsible (0/1)
                    g.setShowDur(1); // Show/Hide Duration (0/1)
                    g.setShowComp(1); // Show/Hide % Complete(0/1)
                    g.setCaptionType('Resource');  // Set to Show Caption (None,Caption,Resource,Duration,Complete)
    ...
    $.fn.project_Gantt = function (method) {
        ...
        if (methods[method]) {
            ...
            $.error('Метод "' + method + '" не найден в плагине jQuery.project_Gantt');
        }
    };
...
JSGantt.TaskItem = function (g, pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pDepend, pCaption) {
